...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 10. Auto-Scroll for Events & Notices ---
    const scrollContainers = document.querySelectorAll('.has-scroll');
    scrollContainers.forEach(container => {
        let isHovered = false;
        
        // Pause scroll on interaction
        container.addEventListener('mouseenter', () => isHovered = true);
        container.addEventListener('mouseleave', () => isHovered = false);
        container.addEventListener('touchstart', () => isHovered = true);
        container.addEventListener('touchend', () => { setTimeout(() => isHovered = false, 1000); });

        const wrap = container.querySelector('.en-items-wrap');
        // Only run auto-scroll if content overflows
        if (wrap && wrap.scrollHeight > container.clientHeight) {
            const originalHeight = wrap.scrollHeight;
            wrap.innerHTML += wrap.innerHTML; // duplicate for infinite effect
            
            let scrollPos = 0;
            function autoScroll() {
                if (!isHovered) {
                    scrollPos += 0.4; // scrolling speed
                    if (scrollPos >= originalHeight) {
                        scrollPos = 0;
                    }
                    container.scrollTop = scrollPos;
                } else {
                    // If user manually scrolls while hovering, update our position tracking
                    // to prevent jumping when they mouseout
                    scrollPos = container.scrollTop;
                    // Note: if they scroll past halfway, we ideally should readjust, but 
                    // for a simple infinite ticker, this works nicely.
                }
