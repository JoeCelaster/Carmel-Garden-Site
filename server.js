...
// Upload Gallery Album (Folder) (Admin only)
app.post('/api/upload-gallery-folder', requireAdmin, upload.array('images'), (req, res) => {
    if (!req.files || req.files.length === 0) {
        return res.status(400).json({ error: 'No files uploaded' });
    }
    const eventName = req.body.eventName || 'New Event';
    try {
        const db = readDB();
        if (!db.gallery) db.gallery = {};
        
        // Ensure album is an object
        if (!db.gallery[eventName]) {
            db.gallery[eventName] = { photos: [], description: '', date: new Date().getFullYear().toString() };
        } else if (Array.isArray(db.gallery[eventName])) {
            db.gallery[eventName] = { photos: db.gallery[eventName], description: '', date: new Date().getFullYear().toString() };
        }
        
        req.files.forEach(file => {
            db.gallery[eventName].photos.push(`images/${file.filename}`);
        });
        writeDB(db);

        res.json({ success: true, message: `Album '${eventName}' uploaded successfully!` });
    } catch (err) {
...
app.post('/api/update-album', requireAdmin, (req, res) => {
    const { oldName, newName, description, date } = req.body;
    try {
        const db = readDB();
        if (!db.gallery || !db.gallery[oldName]) return res.status(404).json({error: 'Album not found'});
        
        const albumData = db.gallery[oldName];
        if (Array.isArray(albumData)) {
            db.gallery[oldName] = { photos: albumData, description: '', date: '' };
        }
        
        db.gallery[oldName].description = description || '';
        db.gallery[oldName].date = date || '';

        // Rename key if changed
        if (newName && newName !== oldName) {
            db.gallery[newName] = db.gallery[oldName];
            delete db.gallery[oldName];
        }
